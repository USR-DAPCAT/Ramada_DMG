...
read_conductor(conductor_cataleg,sheet="periode_ramada") %>% transmute(Año=year,Periodo=paste0(periode1, "  ------  ",periode2),dias=dies) %>% 
...
# Generar dt
dt_ramada_periode<-
  read_conductor(here::here(conductor_cataleg),sheet="periode_ramada") %>% 
...
# dt_temp %>% select(grup_SOG,DG.DMG_recent,grup_DG,grup,DG.DMG)
# 
# dt_temp %>% 
...
```{r M_efectes_mixtes, eval=FALSE,include=FALSE}

# Descriptiva comparativa tenint en compte idp 

# - Models d'efectes mixtes

dt_temp<-dades %>% 
  # filter(n_episodis>=2) %>% 
  select(idp,id_episodi,grup_RAM,SOG50_60.valor,grup)
...
```{r test_paired, eval=FALSE, include=FALSE}
